<template>
    <div :class="[
        'bg-transparent border backdrop-blur-sm rounded-2xl shadow-lg p-6 flex flex-col justify-between h-full transition-all duration-300',
        highlight ? 'border-green-400 ring-2 ring-green-400/30' : 'border-gray-700/30 hover:border-gray-600/50'
    ]">
        <div>
            <!-- Badge para plan destacado -->
            <div v-if="highlight" class="bg-green-500 text-white text-xs font-semibold px-3 py-1 rounded-full mb-3 inline-block">
                POPULAR
            </div>
            <h3 class="text-xl font-semibold mb-2 text-white">{{ name }}</h3>
            <p class="text-2xl font-bold mb-4 text-white">
                {{ formatCurrency(price) }}
                <span class="text-sm font-normal text-gray-400">/ {{ periodicityLabel }}</span>
            </p>
        ...
        <div class="flex flex-col gap-3">
            ...
            <router-link to="/contacto"
                class="border border-gray-600 text-gray-300 hover:bg-gray-700/30 rounded-lg px-6 py-3 text-center font-medium transition-colors duration-200">
                ¿Dudas?
            </router-link>
        </div>
    </div>
