...
export class ProyectosService {
  ...
  async crearProyecto(datosProyecto) {
    try {
      console.log('📝 Creando nuevo proyecto en Firebase...', datosProyecto.titulo)
  ...
  async obtenerProyectosActivos() {
    try {
      ...
      if (resultado.success) {
        const proyectosActivos = resultado.data.filter(proyecto => proyecto.activo !== false)
        return {
          ...resultado,
  ...
  /**
   * Obtener el proyecto estrella
   */
  async obtenerProyectoEstrella() {
    try {
      const resultado = await this.obtenerProyectosActivos()
      if (resultado.success) {
        const proyectoEstrella = resultado.data.find(proyecto => proyecto.esEstrella)
        return {
          success: true,
  ...
  async verificarProyectoEstrella(proyectoIdExcluir = null) {
    try {
      const resultado = await this.obtenerProyectos()
      if (resultado.success) {
        const proyectosEstrella = resultado.data.filter(proyecto => 
          proyecto.esEstrella && proyecto.id !== proyectoIdExcluir
        )
        ...
        for (const proyecto of proyectosEstrella) {
          ...
          console.log(`⭐ Removido estado estrella del proyecto: ${proyecto.titulo}`)
        }
      }
    } catch (error) {
