// Script para probar CV con colores negro FORZADOS con !important
const fs = require("fs");
...
async function probarColoresNegroForzado() {
  console.log("🎨 PROBANDO CV CON COLORES NEGRO FORZADOS (!important)\n");
...
Entregar un CV altamente profesional, adaptado y atractivo tanto para filtros automáticos (ATS) como para reclutadores humanos en tecnología, utilizando al máximo los datos proporcionados y tu experiencia en el área de RRHH técnico.`;
  ...
  // PLANTILLA HTML CON COLORES FORZADOS
...
HTML completo con todas las variables {{}} reemplazadas, optimizado para ATS y reclutadores técnicos, listo para renderizar o convertir a PDF.`;
  ...
  try {
    console.log("\n🤖 Generando CV con colores negro FORZADOS...");
    ...
    fs.writeFileSync("cv-negro-forzado.html", htmlGenerado);
    console.log("💾 CV guardado como: cv-negro-forzado.html");
    ...
    console.log("\n🎯 MEJORAS APLICADAS CON !important:");
...
async function main() {
  ...
  await probarColoresNegroForzado();
  console.log("\n✅ Prueba de colores negro forzados completada");
}
