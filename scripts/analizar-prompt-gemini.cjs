...
<body>
  ...
  console.log(`   - Información personal: ${Object.keys(datosJSON).filter(k => ['nombre_completo', 'email', 'telefono', 'ubicacion'].includes(k)).length} campos`)
  console.log(`   - Experiencia profesional: ${datosJSON.experiencia_profesional.length} trabajos`)
  console.log(`   - Habilidades técnicas: ${Object.keys(datosJSON.habilidades_tecnicas).length} categorías`)
...
Cuando termines, el resultado debe ser un CV listo para renderizarse como HTML o exportarse como PDF.`
  ...
  console.log(`   ✅ Datos JSON: ${JSON.stringify(datosJSON).length} caracteres`)
  ...
  console.log(`   📊 Datos disponibles: ${Object.keys(datosJSON).length} campos principales`)
  
  // 6. ANÁLISIS DE VARIABLES
  console.log('\n🔍 6. ANÁLISIS DE MAPEO VARIABLES → DATOS:')
  console.log('   Verificando qué datos están disponibles para cada variable:\n')
  ...
  const mapeoVariables = {
    ...
    '{{habilidades_tecnicas}}': `${Object.keys(datosJSON.habilidades_tecnicas).length} categorías`,
  ...
  const analisisCompleto = {
    ...
    prompts: {
      ...
      usuario: {
        ...
        componentes: ['plantilla_html', 'datos_json', 'personalizacion', 'instrucciones']
      },
      combinado: {
  ...
  fs.writeFileSync('analisis-prompt-gemini.json', JSON.stringify(analisisCompleto, null, 2))
  console.log('   ✅ Archivo guardado: analisis-prompt-gemini.json')
  
  // 8. RESUMEN FINAL
  console.log('\n📋 8. RESUMEN DEL PROCESO:')
  console.log('=' .repeat(60))
  console.log('   1️⃣ Obtiene plantilla HTML desde Firebase (con variables {{}})')
  ...
  console.log('   4️⃣ Crea prompt usuario (plantilla + datos + personalización)')
